...
public enum AIThresholdEnum {
	...
	StartegyDuation(4,"StartegyDuation",new IAIThreshold(){
		@Override
		public double getThresholdValue(SceneObject self, SceneObject target) {
			return System.currentTimeMillis() - target.getAiCompnent().getCurrentStrategy().getBeginTime();
		}
	}),
	/**
	 * 行为持续时间
	...
	TendencyDuation(5,"TendencyDuation",new IAIThreshold(){
		@Override
		public double getThresholdValue(SceneObject self, SceneObject target) {
			return System.currentTimeMillis() - target.getAiCompnent().getCurrentTendency().getBeginTime();
		}
	}),
	/**
	 * 动作持续时间
	...
	ActionDuation(6,"ActionDuation",new IAIThreshold(){
		@Override
		public double getThresholdValue(SceneObject self, SceneObject target) {
			return System.currentTimeMillis() - target.getAiCompnent().getCurrentAction().getBeginTime();
		}
	}),
	;
