...
--投影到轴平面
--_dropAxis表示需要舍弃的轴，1:x 2:y 3:z
function Quaternion.ProjectToAxisPlane(_self, _dropAxis)
	local _ret = Quaternion.Forward(_self)
	if _dropAxis == 1 then
		_ret.x = 0	
	elseif _dropAxis == 2 then
		_ret.y = 0
	elseif _dropAxis == 3 then
		_ret.z = 0
	end
	return Quaternion.LookRotation(_ret, Vector3.up_global())
end
