...
---@class LuaBehav_RuntimeTask_Select : LuaBehav_RuntimeTask
...
function LuaBehav_RuntimeTask_Select:CheckCanExecute()
	...
	----LogTools.LogByLevel(LogLevel.log, "LuaBehav_RuntimeTask_Select", "CheckCanExecute", "self.curExcuteChildCnt", self.curExcuteChildCnt, "#_stateList", #_stateList)
...
function LuaBehav_RuntimeTask_Select:ReCountTaskState(_taskState)
	...
	for i = 1, #_stateList do
		if not _hideList[i] then
			...
			--	LogTools.LogByLevel(LogLevel.log, "LuaBehav_RuntimeBehavTree", "LuaBehav_RuntimeTask_Select", "ReCountTaskState", "i", i, "_childState", LogTools.GetEnumTableDesc(LuaBehav_Define_TaskState, _childState))
			--end
			...
			if LuaBehav_Define_TaskState.Running == _childState then
				local _index = self.childIndexList[i]
				local _childTask = self.behavTree:GetTask(_index)
				--LogTools.LogByLevel(LogLevel.log, "select child running", i, "child", _childTask.index, _childTask.__taskState)
	...
	--if self.index == 23 then
	--	LogTools.LogByLevel(LogLevel.log, "LuaBehav_RuntimeBehavTree", "LuaBehav_RuntimeTask_Select", "ReCountTaskState end", "final state", LogTools.GetEnumTableDesc(LuaBehav_Define_TaskState, _state))
	--end
