...
--- Describe: 关卡检查单位BUFF(只用于单个关卡单位)
---

---@class LuaBehav_RuntimeTask_LevelCheckMonsBuff : LuaBehav_RuntimeTask
LuaBehav_RuntimeTask_LevelCheckMonsBuff = class(LuaBehav_RuntimeTask, "LuaBehav_RuntimeTask_LevelCheckMonsBuff")

function LuaBehav_RuntimeTask_LevelCheckMonsBuff:IsConditionalTask()
	return true
end

function LuaBehav_RuntimeTask_LevelCheckMonsBuff:OnUpdate(_deltaTime)
	local _retState = LuaBehav_Define_TaskState.Failure
	--LogTools.LogByLevel(LogLevel.logErr, "LuaBehav_RuntimeTask_LevelCheckMonsBuff", "_getValue", _getValue, "self.configTask", dumpTableEx(self.configTask))
	
	local _isUnitBuff = self.configTask.isUnitBuff or false
	
	local _levelUnitId = self.configTask.levelUnitId
	...
	local _netId = _levelUnit.netId
	if not _netId then
		self:__SetTaskState(_retState)
		return _retState
	end
	
	local _checkTarget = self.configTask.checkTarget
	if not _checkTarget or _checkTarget == LuaBehav_Define_MonsCheckBuffTargetType.SelfUnit then
		local _getValue = LuaBehav_RuntimeTask_LevelCheckMonsBuff.__GetBuffStackNum(self.battleId, _netId, self.configTask.buffGroupId, _isUnitBuff)
	...
	elseif _checkTarget == LuaBehav_Define_MonsCheckBuffTargetType.CurTarget then
		...
		if _unit then
			...
			if _behavCom then
				...
				local _getValue = LuaBehav_RuntimeTask_LevelCheckMonsBuff.__GetBuffStackNum(self.battleId, _targetUnitNetId, self.configTask.buffGroupId, _isUnitBuff)
				if self:__Check(self.configTask.compareValue, self.configTask.compareType, _getValue) then
					_retState = LuaBehav_Define_TaskState.Success
				end
			end
		end
	elseif _checkTarget == LuaBehav_Define_MonsCheckBuffTargetType.Subordination then
		local _unit = self.battleRoom.battleUnitManager:GetUnit(_netId)
		if _unit then
			...
			if _subordinationCom then
				...
				if _targetUnit then
					local _getValue = LuaBehav_RuntimeTask_LevelCheckMonsBuff.__GetBuffStackNum(self.battleId, _targetUnit:GetNetId(), self.configTask.buffGroupId, _isUnitBuff)
					if self:__Check(self.configTask.compareValue, self.configTask.compareType, _getValue) then
						_retState = LuaBehav_Define_TaskState.Success
					end
				end
	...
	elseif _checkTarget == LuaBehav_Define_MonsCheckBuffTargetType.AnyInSpecificCamps then
	...
	elseif _checkTarget == LuaBehav_Define_MonsCheckBuffTargetType.AllInSpecificCamps then
		_retState = LuaBehav_Define_TaskState.Success
		for _, _camp in pairs(self.configTask.camps) do
			local _unitList = self.battleRoom.battleUnitManager:GetUnitListByCampType(_camp)
			for _, _targetUnit in pairs(_unitList) do
...
---@private
function LuaBehav_RuntimeTask_LevelCheckMonsBuff:__Check(_compareValue, _compareType, _getValue)
...
---@private
function LuaBehav_RuntimeTask_LevelCheckMonsBuff.__GetBuffStackNum(_battleId, _netId, _buffGroupId, _isUnitBuff)
...
return LuaBehav_RuntimeTask_LevelCheckMonsBuff