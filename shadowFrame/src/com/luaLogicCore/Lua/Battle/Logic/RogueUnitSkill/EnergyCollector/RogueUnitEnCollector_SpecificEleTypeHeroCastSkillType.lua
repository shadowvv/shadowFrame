...
--- DateTime: 2023/07/29 17:36
--- Describe:
---

---@class RogueUnitEnCollector_SpecificEleTypeHeroCastSkillType : table
RogueUnitEnCollector_SpecificEleTypeHeroCastSkillType = class(nil, "RogueUnitEnCollector_SpecificEleTypeHeroCastSkillType")

---@public
---@param _rogueUnitSkill RogueUnitSkillData
---@param _enCollectorXls RogueUnitSkillEnCollector
function RogueUnitEnCollector_SpecificEleTypeHeroCastSkillType:Init(_rogueUnitSkill, _enCollectorXls)
	self:__Init(_rogueUnitSkill, _enCollectorXls)
	
	self.__checkNetId = _rogueUnitSkill:GetUnit():GetNetId()
	self.__checkEleId = math.floor(_enCollectorXls.params[1])
	self.__checkSkillType = math.floor(_enCollectorXls.params[2])
	self:GetBattleRoom().rogueUnitSkillService:AddListener(BATTLE_LOGIC_EVENT_ENUM.USE_SKILL_START, self)
end

---@public
function RogueUnitEnCollector_SpecificEleTypeHeroCastSkillType:OnRemove()
	self:__OnRemove()
...
function RogueUnitEnCollector_SpecificEleTypeHeroCastSkillType:OnTrigEvent(_battleLogicEventV2)
	...
	if CheckLogLevel(LogLevel.log) and LogTools.RogueUnitSkillLog then
		LogTools.LogByLevel(LogLevel.logErr, "肉鸽主公技", "指定属性英雄施放技能增加主公技能量", "英雄", _obj:GetDataInstanceXls().name, "属性类型", self.__addPropId, self.__checkEleId, "技能类型", skillConfig.skillType, "添加属性", self.__addPropStr, "添加数量", self.__addPropNum)
	end
	self.__unitPropCom:IncreasePropByName(self.__addPropStr, self.__addPropNum)
end

return RogueUnitEnCollector_SpecificEleTypeHeroCastSkillType