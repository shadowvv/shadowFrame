...
function ChangePeriodActionState:EnterAction(_actionStateManager)
    self.runTime = 0;
    return true;
end
...
function ChangePeriodActionState:DoAction(_actionStateManager, _dt, _netId, _objId)
    if self.runTime > self.stateParam.changePeriodTime then
...
function ChangePeriodActionState:ExitAction(_actionStateManager,_isBreak,_isActive)

end

-- 是否为主动状态
---@return boolean 是否为主动状态
function ChangePeriodActionState:IsActive()
    return true;
end

-- 行为状态Id
---@return number 行为状态Id 参照stateSwitchRule表
function ChangePeriodActionState:GetId()
    return 14;
end

---获得状态冲突表
---@return StateSwitchRule
function ChangePeriodActionState:GetSwitchRule()
    return self.battleRoom.inputDataSource:GetDict("StateSwitchRule",self:GetId());
end

return ChangePeriodActionState;