...
---@class BattleLogicBattleRunStartEvent : BattleLogicEventV2
...
BattleLogicBattleRunStartEvent = class(BattleLogicEventV2, 'BattleLogicBattleRunStartEvent');
...
function BattleLogicBattleRunStartEvent:ctor(_eventType, _eventCompareParam, _eventTriggerParam, _netId, _objectId, _battleId)
...
function BattleLogicBattleRunStartEvent:CheckEventTrigger(_eventCompareParam)
    return true;
end

---事件回调，默认为空
---@public
function BattleLogicBattleRunStartEvent:FireEvent()
    if self.survival == 0 then
        ...
        _levelOper:CheckPlayerTriggerInitConditions()
    end
end
...
---@return boolean 是否互斥
function BattleLogicBattleRunStartEvent:CheckMutexEvent(_event)
    return false;
end

---检测移除事件，默认事件会在trigger之后的下一帧被移除
---@public
function BattleLogicBattleRunStartEvent:CheckRemoveEvent()
...
---生成比较参数
---@public
function BattleLogicBattleRunStartEvent:GenerateCompareParam(_param);
    local compareParam = {};
    return compareParam;
end

return BattleLogicBattleRunStartEvent;