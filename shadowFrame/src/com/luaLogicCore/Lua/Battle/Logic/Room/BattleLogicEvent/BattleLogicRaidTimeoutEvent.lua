...
---@class BattleLogicRaidTimeoutEvent : BattleLogicEventV2
...
BattleLogicRaidTimeoutEvent = class(BattleLogicEventV2, 'BattleLogicRaidTimeoutEvent');
...
function BattleLogicRaidTimeoutEvent:ctor(_eventType, _eventCompareParam, _eventTriggerParam, _netId, _objectId, _battleId)
...
function BattleLogicRaidTimeoutEvent:CheckEventTrigger(_eventCompareParam)
    return true;
end

---事件回调，默认为空
---@public
function BattleLogicRaidTimeoutEvent:FireEvent()
...
---@return boolean 是否互斥
function BattleLogicRaidTimeoutEvent:CheckMutexEvent(_event)
    return false;
end

---检测移除事件，默认事件会在trigger之后的下一帧被移除
---@public
function BattleLogicRaidTimeoutEvent:CheckRemoveEvent()
...
---生成比较参数
---@public
function BattleLogicRaidTimeoutEvent:GenerateCompareParam(_param);
    local compareParam = {};
    return compareParam;
end

return BattleLogicRaidTimeoutEvent;