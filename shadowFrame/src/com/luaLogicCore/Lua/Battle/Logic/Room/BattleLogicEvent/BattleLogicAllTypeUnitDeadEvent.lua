...
---@class BattleLogicAllTypeUnitDeadEvent : BattleLogicEventV2
...
BattleLogicAllTypeUnitDeadEvent = class(BattleLogicEventV2, 'BattleLogicAllTypeUnitDeadEvent');
...
function BattleLogicAllTypeUnitDeadEvent:ctor(_eventType, _eventCompareParam, _eventTriggerParam, _netId, _objectId, _battleId)
...
function BattleLogicAllTypeUnitDeadEvent:CheckEventTrigger(_eventCompareParam)
    return self.eventCompareParam.type == _eventCompareParam.type;
end

---事件回调，默认为空
---@public
function BattleLogicAllTypeUnitDeadEvent:FireEvent()
...
---@return boolean 是否互斥
function BattleLogicAllTypeUnitDeadEvent:CheckMutexEvent(_event)
    return false;
end

---检测移除事件，默认事件会在trigger之后的下一帧被移除
---@public
function BattleLogicAllTypeUnitDeadEvent:CheckRemoveEvent()
...
---生成比较参数
---@public
function BattleLogicAllTypeUnitDeadEvent:GenerateCompareParam(_param);
    ---compareParam.type 单位类型
    local compareParam = {};
    compareParam.type = tonumber(_param);
    return compareParam;
end

return BattleLogicAllTypeUnitDeadEvent;