...
---@class BattleLogicHitEvent : BattleLogicEventV2
...
BattleLogicHitEvent = class(BattleLogicEventV2, 'BattleLogicHitEvent');
...
function BattleLogicHitEvent:ctor(_eventType, _eventCompareParam, _eventTriggerParam, _netId, _objectId, _battleId)
...
function BattleLogicHitEvent:CheckEventTrigger(_eventCompareParam)
    return true;
end

---事件回调，默认为空
---@public
function BattleLogicHitEvent:FireEvent()
...
---@return boolean 是否互斥
function BattleLogicHitEvent:CheckMutexEvent(_event)
    return false;
end

---检测移除事件，默认事件会在trigger之后的下一帧被移除
---@public
function BattleLogicHitEvent:CheckRemoveEvent()
...
---生成比较参数
---@public
function BattleLogicHitEvent:GenerateCompareParam(_param);
    local compareParam = {};
    return compareParam;
end


return BattleLogicHitEvent;