...
---@class BattleLogicAddBuffEvent : BattleLogicEventV2
...
BattleLogicAddBuffEvent = class(BattleLogicEventV2, 'BattleLogicAddBuffEvent');
...
function BattleLogicAddBuffEvent:ctor(_eventType, _eventCompareParam, _eventTriggerParam, _netId, _objectId, _battleId)
...
function BattleLogicAddBuffEvent:CheckEventTrigger(_eventCompareParam)
    return self.eventCompareParam.buffId == _eventCompareParam.buffId;
end

---事件回调，默认为空
---@public
function BattleLogicAddBuffEvent:FireEvent()
    if self.survival == 0 then
        local battleRoom = GetBattleRoom(self.battleId)
        ---@type BattleUnitBase
        local _unit = battleRoom.battleUnitManager:GetUnit(self.eventCompareParam.castNetId);
        if _unit then
            for i, v in pairs(_unit:GetAllObjectDic()) do
                ...
                if skillComp then
                    skillComp:OnAddBuffToOther(self.eventCompareParam.buffId,self.netId,self.objectId);
                end
            end
        end
    end
...
---@return boolean 是否互斥
function BattleLogicAddBuffEvent:CheckMutexEvent(_event)
    return false;
end

---检测移除事件，默认事件会在trigger之后的下一帧被移除
---@public
function BattleLogicAddBuffEvent:CheckRemoveEvent()
...
return BattleLogicAddBuffEvent;