...
function BeHitActionState:EnterAction(_actionStateManager)
    ...
    if buffComp then
        buffComp:AddBattleBuffById(13, nil, nil);
    end
    return true;
end
...
function BeHitActionState:DoAction(_actionStateManager, _dt, _netId)
    if self.runTime > self.stateParam.beHitTime then
...
function BeHitActionState:ExitAction(_actionStateManager,_isBreak,_isActive)

end

-- 是否为主动状态
---@return boolean 是否为主动状态
function BeHitActionState:IsActive()
    return false;
end

-- 行为状态Id
---@return number 行为状态Id 参照stateSwitchRule表
function BeHitActionState:GetId()
    return 8;
end

---获得状态冲突表
---@return StateSwitchRule
function BeHitActionState:GetSwitchRule()
    return self.battleRoom.inputDataSource:GetDict("StateSwitchRule",self:GetId());
end

return BeHitActionState;