...
---@class BattleLogicChangeLeaderEvent : BattleLogicEventV2
...
BattleLogicChangeLeaderEvent = class(BattleLogicEventV2, 'BattleLogicChangeLeaderEvent');
...
function BattleLogicChangeLeaderEvent:ctor(_eventType, _eventCompareParam, _eventTriggerParam, _netId, _objectId, _battleId)
...
function BattleLogicChangeLeaderEvent:CheckEventTrigger(_eventCompareParam)
    return true
end

---事件回调，默认为空
---@public
function BattleLogicChangeLeaderEvent:FireEvent()
    --eventTriggerParam.oldObjectId
    --eventTriggerParam.newObjectId
    if self.survival == 0 then
        local battleRoom = GetBattleRoom(self.battleId)
        ---@type BattleUnitBase
        local unit = battleRoom.battleUnitManager:GetUnit(self.netId);
        if unit then
            ---@type BattleUnitCom_Buff
            local buffComp = unit:GetComponent(BattleUnitComponentType.Buff);
            if buffComp then
                buffComp:OnObjectStandBy(self.eventTriggerParam.oldObjectId,self.eventTriggerParam.newObjectId);
            end
            ---@type BattleObjectBase
            local object = unit:GetHeroObject(self.eventTriggerParam.oldObjectId);
            if object then
                ...
                local PlayComp = object:GetComponent(BattleObjectComponentType.PlayMode);
                if PlayComp then
                    PlayComp:ChangeMode(1);
                end
            end
        end
    end
...
---@return boolean 是否互斥
function BattleLogicChangeLeaderEvent:CheckMutexEvent(_event)
    return false;
end

---检测移除事件，默认事件会在trigger之后的下一帧被移除
---@public
function BattleLogicChangeLeaderEvent:CheckRemoveEvent()
...
---生成比较参数
---@public
function BattleLogicChangeLeaderEvent:GenerateCompareParam(_param);
    local compareParam = {};
    return compareParam;
end

return BattleLogicChangeLeaderEvent;