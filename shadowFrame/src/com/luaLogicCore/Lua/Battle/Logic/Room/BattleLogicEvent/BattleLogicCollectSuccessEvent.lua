...
---@class BattleLogicCollectSuccessEvent : BattleLogicEventV2
...
BattleLogicCollectSuccessEvent = class(BattleLogicEventV2, "BattleLogicCollectSuccessEvent")
...
function BattleLogicCollectSuccessEvent:ctor(_eventType, _eventCompareParam, _eventTriggerParam, _netId, _objectId, _battleId)
...
function BattleLogicCollectSuccessEvent:CheckEventTrigger(_eventCompareParam)
    return true
end

---事件回调，默认为空
---@public
function BattleLogicCollectSuccessEvent:FireEvent()
    if self.survival == 0 then
        local _battleRoom = GetBattleRoom(self.battleId)
        _battleRoom.dataCollectManager:OnBattleCollectSuccess(
                self.eventTriggerParam.targetNetId,
                self.eventTriggerParam.targetObjId,
                self.eventTriggerParam.fromNetId,
                self.eventTriggerParam.fromObjId
        )
    end
end
...
---@return boolean 是否互斥
function BattleLogicCollectSuccessEvent:CheckMutexEvent(_event)
    return false;
end

---检测移除事件，默认事件会在trigger之后的下一帧被移除
---@public
function BattleLogicCollectSuccessEvent:CheckRemoveEvent()
...
return BattleLogicCollectSuccessEvent