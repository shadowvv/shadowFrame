...
---@class BattleLogicBuffOverEvent : BattleLogicEventV2
...
BattleLogicBuffOverEvent = class(BattleLogicEventV2, 'BattleLogicBuffOverEvent');
...
function BattleLogicBuffOverEvent:ctor(_eventType, _eventCompareParam, _eventTriggerParam, _netId, _objectId, _battleId)
...
function BattleLogicBuffOverEvent:CheckEventTrigger(_eventCompareParam)
    return self.eventCompareParam.buffId == _eventCompareParam.buffId;
end

---事件回调，默认为空
---@public
function BattleLogicBuffOverEvent:FireEvent()
    if self.survival == 0 then
        ...
        if _unit then
            ...
            if _object then
                ...
                if skillComp then
                    skillComp:OnBuffOver(self.netId,self.objectId,self.eventCompareParam.buffId,self.eventCompareParam.stackNum,self.eventCompareParam.isTimeOver);
                end
            end
        end
    end
...
---@return boolean 是否互斥
function BattleLogicBuffOverEvent:CheckMutexEvent(_event)
    return false;
end

---检测移除事件，默认事件会在trigger之后的下一帧被移除
---@public
function BattleLogicBuffOverEvent:CheckRemoveEvent()
...
return BattleLogicBuffOverEvent;