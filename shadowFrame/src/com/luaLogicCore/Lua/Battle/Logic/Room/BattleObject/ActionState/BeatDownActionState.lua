...
function BeatDownActionState:EnterAction(_actionStateManager)
    ...
    if buffComp then
        buffComp:AddBattleBuffById(14, nil, nil);
    end
    return true;
end
...
function BeatDownActionState:DoAction(_actionStateManager, _dt, _netId)
    if self.runTime > self.stateParam.beatDownTime then
...
function BeatDownActionState:ExitAction(_actionStateManager,_isBreak,_isActive)

end

-- 是否为主动状态
---@return boolean 是否为主动状态
function BeatDownActionState:IsActive()
    return false;
end

-- 行为状态Id
---@return number 行为状态Id 参照stateSwitchRule表
function BeatDownActionState:GetId()
    return 9;
end

---获得状态冲突表
---@return StateSwitchRule
function BeatDownActionState:GetSwitchRule()
    return self.battleRoom.inputDataSource:GetDict("StateSwitchRule",self:GetId());
end

return BeatDownActionState;