...
---@class BattleUnitCollideEvent : BattleLogicEventV2
...
BattleUnitCollideEvent = class(BattleLogicEventV2, 'BattleUnitCollideEvent');
...
function BattleUnitCollideEvent:ctor(_eventType, _eventCompareParam, _eventTriggerParam, _netId, _objectId, _battleId)
...
function BattleUnitCollideEvent:CheckEventTrigger(_eventCompareParam)
    return self.eventCompareParam.eventId == _eventCompareParam.eventId;
end

---事件回调，默认为空
---@public
function BattleUnitCollideEvent:FireEvent()
...
---@return boolean 是否互斥
function BattleUnitCollideEvent:CheckMutexEvent(_event)
    return false;
end

---检测移除事件，默认事件会在trigger之后的下一帧被移除
---@public
function BattleUnitCollideEvent:CheckRemoveEvent()
