...
---@class BattleLogicCasterAddBuffEvent : BattleLogicEventV2
...
BattleLogicCasterAddBuffEvent = class(BattleLogicEventV2, 'BattleLogicCasterAddBuffEvent');
...
function BattleLogicCasterAddBuffEvent:ctor(_eventType, _eventCompareParam, _eventTriggerParam, _netId, _objectId, _battleId)
...
function BattleLogicCasterAddBuffEvent:CheckEventTrigger(_eventCompareParam)
    return self.eventCompareParam.buffId == _eventCompareParam.buffId;
end

---事件回调，默认为空
---@public
function BattleLogicCasterAddBuffEvent:FireEvent()
...
---@return boolean 是否互斥
function BattleLogicCasterAddBuffEvent:CheckMutexEvent(_event)
    return false;
end

---检测移除事件，默认事件会在trigger之后的下一帧被移除
---@public
function BattleLogicCasterAddBuffEvent:CheckRemoveEvent()
...
return BattleLogicCasterAddBuffEvent;