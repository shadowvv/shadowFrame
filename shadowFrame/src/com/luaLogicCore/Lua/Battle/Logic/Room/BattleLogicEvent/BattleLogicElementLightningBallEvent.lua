...
---@class BattleLogicElementLightningBallEvent : BattleLogicEventV2
...
BattleLogicElementLightningBallEvent = class(BattleLogicEventV2, 'BattleLogicElementLightningBallEvent');
...
function BattleLogicElementLightningBallEvent:ctor(_eventType, _eventCompareParam, _eventTriggerParam, _netId, _objectId, _battleId)
...
function BattleLogicElementLightningBallEvent:CheckEventTrigger(_eventCompareParam)
    return self.eventCompareParam.buffId == _eventCompareParam.buffId;
end

---事件回调，默认为空
---@public
function BattleLogicElementLightningBallEvent:FireEvent()
    if self.survival == 0 then
        local battleRoom = GetBattleRoom(self.battleId)
        battleRoom.dataCollectManager:OnTriggerElementLightningBall(self.netId, self.objectId)
    end
end
...
---@return boolean 是否互斥
function BattleLogicElementLightningBallEvent:CheckMutexEvent(_event)
    return false;
end

---检测移除事件，默认事件会在trigger之后的下一帧被移除
---@public
function BattleLogicElementLightningBallEvent:CheckRemoveEvent()
...
return BattleLogicElementLightningBallEvent;