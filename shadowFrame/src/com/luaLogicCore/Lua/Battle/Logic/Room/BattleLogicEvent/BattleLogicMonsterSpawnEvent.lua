...
---@class BattleLogicMonsterSpawnEvent : BattleLogicEventV2
...
BattleLogicMonsterSpawnEvent = class(BattleLogicEventV2, 'BattleLogicMonsterSpawnEvent');
...
function BattleLogicMonsterSpawnEvent:ctor(_eventType, _eventCompareParam, _eventTriggerParam, _netId, _objectId, _battleId)
...
function BattleLogicMonsterSpawnEvent:CheckEventTrigger(_eventCompareParam)
    return true;
end

---事件回调，默认为空
---@public
function BattleLogicMonsterSpawnEvent:FireEvent()
...
---@return boolean 是否互斥
function BattleLogicMonsterSpawnEvent:CheckMutexEvent(_event)
    return false;
end

---检测移除事件，默认事件会在trigger之后的下一帧被移除
---@public
function BattleLogicMonsterSpawnEvent:CheckRemoveEvent()
...
---生成比较参数
---@public
function BattleLogicMonsterSpawnEvent:GenerateCompareParam(_param);
    local compareParam = {};
    return compareParam;
end

return BattleLogicMonsterSpawnEvent;