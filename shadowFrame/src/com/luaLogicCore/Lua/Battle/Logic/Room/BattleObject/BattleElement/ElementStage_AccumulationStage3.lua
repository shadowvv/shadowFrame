...
---@class ElementStage_AccumulationStage3 : ElementStageBase
ElementStage_AccumulationStage3 = class(ElementStageBase, 'ElementStage_AccumulationStage3');

function ElementStage_AccumulationStage3:Init(_index, _stageManager)
    self:__Init(_index, _stageManager)
    local _elementStackEffect = self.elementStackEffect
    self.initSpeed = _elementStackEffect.reduceSpeed[BATTLE_ELEMENT_CONST.ELEMENT_REDUCE_SPEED_K_INDEX] or 0
    self.speedAccTime = _elementStackEffect.reduceSpeed[BATTLE_ELEMENT_CONST.ELEMENT_REDUCE_TIME_C_INDEX] or 0
    self.maxSpeed = _elementStackEffect.reduceSpeed[BATTLE_ELEMENT_CONST.ELEMENT_REDUCE_SPEED_L_INDEX] or 0
    self.curSpeed = self.initSpeed
    self.reduceSpeedAcc = (self.maxSpeed - self.initSpeed) / self.speedAccTime
end

function ElementStage_AccumulationStage3:OnEnter()
    self:__OnEnter()
    if CheckLogLevel(LogLevel.log) and LogTools.ElementLog then
        ...
        LogTools.LogByLevel(LogLevel.log, "元素", "元素[" .. GetBattleElementNameById(self.elementId) .. "]", "目标" .. _targetObjectLog, "<color=#00FF00>进入加速衰减阶段</color>",
...
--- 获取衰减速度
function ElementStage_AccumulationStage3:GetReduceSpeed()
...
---@return number 元素量
function ElementStage_AccumulationStage3:__ConvertTimeToValue(_time)
    -- 加速衰减时间
    local _accTime = Mathf.Min(self.speedAccTime - self.runTime, 0)
    -- 匀速衰减时间
...
---@return number 时间
function ElementStage_AccumulationStage3:__ConvertValueToTime(_value)
    local _maxValue = self.stateManager.battleObjComElement:GetElementMaxValue(self.elementId)
    local _time = 0
...
-- 元素改变
---@public
function ElementStage_AccumulationStage3:OnElementChange(_newValue, _oldValue)
