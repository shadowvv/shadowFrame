...
--- DateTime: 2022/10/14 17:02

require "Lib/class"
require "Battle/Logic/Buff/BattleBuffEffect"

---@class ChangeSkillHurtParamBattleBuffFeect : BattleBuffEffect
...
ChangeSkillHurtParamBattleBuffEffect = class(BattleBuffEffect, 'ChangeSkillHurtParamBattleBuffEffect');

function ChangeSkillHurtParamBattleBuffEffect:ctor(_buffInstanceId)
...
function ChangeSkillHurtParamBattleBuffEffect:Trigger(_battleObject, _battleBuff, _stackNum,_isUnitChangeLeader)
    ...
    local skillId = buffAndActionTemplate.behindParam[4];
    local index = buffAndActionTemplate.behindParam[5];

    ---@type BattleObjCom_Skill
    local skillComp = _battleObject:GetComponent(BattleObjectComponentType.Skill);
    if skillComp then
        skillComp:ChangeSkillHurtParam(skillId,index,self:GetCustomParam(_battleObject));
    end
end
...
function ChangeSkillHurtParamBattleBuffEffect:Release(_battleObject, _battleBuff, _stackNum, _isTimeOver)
    ---@type BuffAndActionInstance
    local buffAndActionInstance = _battleObject:GetBattleRoom().inputDataSource:GetDict("BuffAndActionInstance",self.buffInstanceId);
    ---@type BuffAndActionTemplate
    local buffAndActionTemplate = _battleObject:GetBattleRoom().inputDataSource:GetDict("BuffAndActionTemplate",buffAndActionInstance.templateId);
    ...
    local skillComp = _battleObject:GetComponent(BattleObjectComponentType.Skill);
    if skillComp then
        skillComp:ChangeSkillHurtParam(skillId,index,nil);
    end
end

---@public
---@return number buff类型Id
function ChangeSkillHurtParamBattleBuffEffect:GetId()
    return 0;
end
...
---@return table 施法者参数
function ChangeSkillHurtParamBattleBuffEffect:GenerateCostCustomParam(_battleObjectBase, _buffInstanceId, _param)
    return _param;
end

return ChangeSkillHurtParamBattleBuffEffect;