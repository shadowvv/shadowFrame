...
--- DateTime: 2022/7/15 18:16

require "Lib/class"
require "Battle/Logic/Buff/BattleBuffEffect"

---@class SummonBattleBuffEffect : BattleBuffEffect
...
SummonBattleBuffEffect = class(BattleBuffEffect, 'SummonBattleBuffEffect');

function SummonBattleBuffEffect:ctor(_buffInstanceId)
...
function SummonBattleBuffEffect:Trigger(_battleObject, _battleBuff, _stackNum,_isUnitChangeLeader)
    ...
    local summonId, _ = math.modf(buffAndActionTemplate.behindParam[4])
    _battleObject:GetBattleRoom().behaviorProcessor:CreateSummonedMonsUnit(summonId,_battleBuff.castUnitId,_battleBuff.castObjId, _battleBuff.castSkillId);
end
...
---@param _battleBuff BattleBuff buff实体
---@param _stackNum number 堆叠数量
function SummonBattleBuffEffect:Release(_battleObject, _battleBuff, _stackNum)

end

---@public
---@return number buff类型Id
function SummonBattleBuffEffect:GetId()
    return 41;
end
...
---@return table 施法者参数
function SummonBattleBuffEffect:GenerateCostCustomParam(_battleObjectBase,_buffInstanceId,_param)
    return _param;
end

return SummonBattleBuffEffect;