...
---@class FireElementBattleBuffEffect : BattleBuffEffect
...
FireElementBattleBuffEffect = class(BattleBuffEffect, 'FireElementBattleBuffEffect');

function FireElementBattleBuffEffect:ctor(_buffInstanceId)
...
function FireElementBattleBuffEffect:Trigger(_battleObject, _battleBuff, _stackNum, _isUnitChangeLeader)
    ...
    --local _elementId = BattleElementTypeEnum.FIRE:GetId(); --元素类型
	...
	_hurtStackData.directHurtNum = _snapShotData:GetCalcUseAtk() * ( 1 + _stackNum * 0.2)
...
function FireElementBattleBuffEffect:Release(_battleObject, _battleBuff, _stackNum, _isTimeOver)

end

---@public
---@return number buff类型Id
function FireElementBattleBuffEffect:GetId()
    return 69;
end
...
---@return table 施法者参数
function FireElementBattleBuffEffect:GenerateCostCustomParam(_battleObjectBase, _buffInstanceId, _param)
    ...
    if _unit then
        ...
        local _checkElementId = buffAndActionTemplate.behindParam[3] --元素类型
...
return FireElementBattleBuffEffect;