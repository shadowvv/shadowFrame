...
--- DateTime: 2023/5/30 18:53

require "Lib/class"
require "Battle/Logic/Buff/BattleBuffEffect"

---@class ElementExplodeBattleBuffEffect : BattleBuffEffect
...
ElementExplodeBattleBuffEffect = class(BattleBuffEffect, 'ElementExplodeBattleBuffEffect');

function ElementExplodeBattleBuffEffect:ctor(_buffInstanceId)
...
function ElementExplodeBattleBuffEffect:Trigger(_battleObject, _battleBuff, _stackNum, _isUnitChangeLeader)
    ...
    local _specifiedEleBreakId = _battleBuff.generateStackInfo and _battleBuff.generateStackInfo.specifiedEleBreakId or 0
    ...
    if LogTools.HurtLog and CheckLogLevel(LogLevel.log) then
        LogTools.LogByLevel(LogLevel.log, "CheckHurt", "触发元素击破爆炸伤害buff", "来源NetId", _snapShotData:GetNetId(), "来源ObjId", _snapShotData:GetObjId(), "目标NetId", _battleObject:GetParentUnit():GetNetId(), "目标ObjId", _battleObject:GetObjectId(), "buffGroupId", _battleBuff.buffGroupId, "buffInstanceId", self.buffInstanceId, "buffTemplateId", buffAndActionInstance.templateId, "堆叠层数", _stackNum, "_battleBuff.bodyPartId", _battleBuff.bodyPartId, "最终分块ID", _blockId, "_battleBuff.generateStackInfo.specifiedEleBreakId", _specifiedEleBreakId, "本次爆炸伤害属性类型", _elementId, "buffAndActionTemplate.behindParam[5]", buffAndActionTemplate.behindParam[5])
    end
    ...
    _hurtStackData.eleBreakId = _specifiedEleBreakId
...
function ElementExplodeBattleBuffEffect:Release(_battleObject, _battleBuff, _stackNum, _isTimeOver)

end

---@public
---@return number buff类型Id
function ElementExplodeBattleBuffEffect:GetId()
    return 68;
end
...
---@return table 施法者参数
function ElementExplodeBattleBuffEffect:GenerateCostCustomParam(_battleObjectBase, _buffInstanceId, _param)
...
return ElementExplodeBattleBuffEffect;