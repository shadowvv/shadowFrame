...
--- DateTime: 2022/12/2 13:11

require "Lib/class"
require "Battle/Logic/Buff/BattleBuffEffect"

---@class ChangeTypePlayModeIdBattleBuffEffect : BattleBuffEffect
...
ChangeTypePlayModeIdBattleBuffEffect = class(BattleBuffEffect, 'ChangeTypePlayModeIdBattleBuffEffect');

function ChangeTypePlayModeIdBattleBuffEffect:ctor(_buffInstanceId)
...
function ChangeTypePlayModeIdBattleBuffEffect:Trigger(_battleObject, _battleBuff, _stackNum,_isUnitChangeLeader)
    ...
    local playMode = _battleObject:GetComponent(BattleObjectComponentType.PlayMode);
    if playMode then
        playMode:ChangeModeId(math.modf(buffAndActionTemplate.behindParam[4]),math.modf(buffAndActionTemplate.behindParam[5]),math.modf(buffAndActionTemplate.behindParam[6]));
    end
end
...
function ChangeTypePlayModeIdBattleBuffEffect:Release(_battleObject, _battleBuff, _stackNum, _isTimeOver)
    ---@type BuffAndActionInstance
    local buffAndActionInstance = _battleObject:GetBattleRoom().inputDataSource:GetDict("BuffAndActionInstance",self.buffInstanceId);
    ---@type BuffAndActionTemplate
    local buffAndActionTemplate = _battleObject:GetBattleRoom().inputDataSource:GetDict("BuffAndActionTemplate",buffAndActionInstance.templateId);
    ...
    local playMode = _battleObject:GetComponent(BattleObjectComponentType.PlayMode);
    if playMode then
        playMode:ChangeModeId(math.modf(buffAndActionTemplate.behindParam[4]),nil,math.modf(buffAndActionTemplate.behindParam[6]));
    end
end

---@public
---@return number buff类型Id
function ChangeTypePlayModeIdBattleBuffEffect:GetId()
    return 58;
end
...
---@return table 施法者参数
function ChangeTypePlayModeIdBattleBuffEffect:GenerateCostCustomParam(_battleObjectBase, _buffInstanceId, _param)
    return _param;
end

return ChangeTypePlayModeIdBattleBuffEffect;