...
--- DateTime: 2022/6/21 17:18

require "Lib/class"
require "Battle/Logic/Buff/BattleBuffEffect"

---@class SkillCoolDownBattleBuffEffect : BattleBuffEffect
...
SkillCoolDownBattleBuffEffect = class(BattleBuffEffect, 'SkillCoolDownBattleBuffEffect');

function SkillCoolDownBattleBuffEffect:ctor(_buffInstanceId)
...
function SkillCoolDownBattleBuffEffect:Trigger(_battleObject, _battleBuff, _stackNum,_isUnitChangeLeader)
    ...
    local _skillComp = _battleObject:GetComponent(BattleObjectComponentType.Skill);
    if not _skillComp then
        return;
    end

    local _behindParam = buffTemplate.behindParam;
    _skillComp:ChangeSkillCoolDownBySkillType(_behindParam[4],_behindParam[5]*_stackNum*(1+_battleBuff.upPercent[buffTemplate.metaType]));
end
...
---@param _battleBuff BattleBuff buff实体
---@param _stackNum number 堆叠数量
function SkillCoolDownBattleBuffEffect:Release(_battleObject, _battleBuff, _stackNum)

end

---@public
---@return number buff类型Id
function SkillCoolDownBattleBuffEffect:GetId()
    return 35;
end
...
---@return table 施法者参数
function SkillCoolDownBattleBuffEffect:GenerateCostCustomParam(_battleObjectBase,_buffInstanceId,_param)
    return _param;
end

return SkillCoolDownBattleBuffEffect;